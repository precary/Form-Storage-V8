angular.module('umbraco')
    .controller('FormStorageV8.FormStorageV8Controller',
		function ($scope,assetsService,editorState) {
			$('#loadingLabel').hide();
			$.ajax({
				type: 'GET',
				url: '/Umbraco/Surface/FormStorageV8/GetFieldsForAlias/?alias=' + escape($scope.model.config.alias),
				datatype: 'json',
				...
				success: function (result) {
					...
					$('#downloadButton').click(function() {
						...
						window.location = '/Umbraco/Surface/FormStorageV8/DownloadFormSubmissions/?alias=' + escape($scope.model.config.alias) + filename + filterString + sortingString;
					});
	...
	$('#submissionsGrid').jsGrid({
		...
		controller: {
            ...
            loadData: function (filter) {
				...
				return $.ajax({
					type: 'GET',
					url: '/Umbraco/Surface/FormStorageV8/FetchFormSubmissionData/?alias=' + escape(alias),
					data: filter,
					dataType: 'json'
				});
			},
			...
			deleteItem: function (item) {
				...
				$.ajax({
					type: 'DELETE',
					url: '/Umbraco/Surface/FormStorageV8/DeleteFormSubmissionRecord/?submissionID=' + item.submissionID,
					dataType: 'json'
